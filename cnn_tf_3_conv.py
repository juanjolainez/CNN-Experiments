...
def main():
    ...
    W21_shape = (5, 5, 50, 100) # (filter_width, filter_height, old_num_feature_maps, num_feature_maps)
    W21_init = init_filter(W21_shape, poolsz)
    b21_init = np.zeros(W21_shape[-1], dtype=np.float32)

    # vanilla ANN weights
    W3_init = np.random.randn(W21_shape[-1]*4*4, M) / np.sqrt(W21_shape[-1]*4*4 + M)
    ...
    W21 = tf.Variable(W21_init.astype(np.float32))
    b21 = tf.Variable(b21_init.astype(np.float32))
    ...
    Z1 = convpool(X, W1, b1)
    Z1_shape = Z1.get_shape().as_list()
    print Z1_shape
    Z2 = convpool(Z1, W2, b2)
    Z2_shape = Z2.get_shape().as_list()
    print Z2_shape
    Z21 = convpool(Z2, W21, b21)
    Z21_shape = Z21.get_shape().as_list()
    print Z21_shape
    Z21r = tf.reshape(Z21, [Z21_shape[0], np.prod(Z21_shape[1:])])
    print Z21r.shape
    print W3.shape
    Z3 = tf.nn.relu( tf.matmul(Z21r, W3) + b3 )
